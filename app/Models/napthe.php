...
class napthe extends ModelsModel
{
   public $db;
   ...
   public function lichsunap($loai_the, $menh_gia, $mathe, $seri, $trang_thai)
   {
      ...
      if (isset($_POST['submit_nap']) && isset($_SESSION['ma_user'])) {
         if (!isset($_POST['telco']) || !isset($_POST['amount']) || !isset($_POST['serial']) || !isset($_POST['code'])) {
            ...
            if ($obj->status == 99) {
               ...
               $this->db->pdo_execute($sql_addtien);
            } elseif ($obj->status == 1) {
               //Thành công
               thongbao('Thành công');
               // echo '<pre>';
               ...
               $this->db->pdo_execute($sql_addtien, $dataPost['amount'], $_SESSION['ma_user']);
            } elseif ($obj->status == 2) {
               ...
               $this->lichsunap($dataPost['telco'],  $dataPost['amount'], $dataPost['code'], $dataPost['serial'], 1);
               $sql_addtien = "UPDATE `user` SET `tien`=? WHERE  ma_kh = ?";
               $this->db->pdo_execute($sql_addtien, (($dataPost['amount']) * 0.5), $_SESSION['ma_user']);
            } elseif ($obj->status == 3) {
               //Thẻ lỗi
